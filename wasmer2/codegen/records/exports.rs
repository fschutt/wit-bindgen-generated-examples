pub mod records {
    ...
    pub trait Records: Sized + wasmer::WasmerEnv + 'static {
        ...
        T: Records,
    {
        ...
        impl<T: Records> wasmer::WasmerEnv for EnvWrapper<T> {
        ...
        exports.insert("empty-arg", wasmer::Function::new_native_with_env(store, env.clone(), move |env: &std::sync::Arc<std::sync::Mutex<EnvWrapper<T>>>| -> Result<(), wasmer::RuntimeError> {
      let env = &mut *env.lock().unwrap();
      let host = &mut env.data;
      let param0 = Empty{};
      ...
      Ok(())
        ...
        exports.insert("empty-result", wasmer::Function::new_native_with_env(store, env.clone(), move |env: &std::sync::Arc<std::sync::Mutex<EnvWrapper<T>>>| -> Result<(), wasmer::RuntimeError> {
      let env = &mut *env.lock().unwrap();
      let host = &mut env.data;
      ...
      Ok(())
        ...
        imports.register("records", exports);
    }
